...
fn discover_glb_fixtures(repo_root: &Path) -> Vec<PathBuf> {
    let glb_dir = repo_root.join("assets/glb");
    let Ok(entries) = fs::read_dir(&glb_dir) else {
        return Vec::new();
    };
    ...
    let mut paths = entries
        ...
        .filter(|path| path.extension().and_then(|ext| ext.to_str()) == Some("glb"))
        .collect::<Vec<_>>();
    paths.sort();
    paths
}
...
fn render_case(
    ...
    let stats = render_frame_with_backend(
        ...
        frame_pipeline.material_morph_weights(),
        &glyph_ramp,
        &mut scratch,
        Camera::default(),
        0.22,
...
fn parity_metrics(
    ...
    ParityMetrics {
        ...
        visible_ratio_delta: (cpu_stats.visible_cell_ratio - gpu_stats.visible_cell_ratio).abs(),
    }
}
...
fn gpu_morph_cache_shared_mesh_instances_are_isolated() {
    ...
    let (cpu_frame, cpu_stats) = render_case(crate::scene::RenderBackend::Cpu, &scene, cfg.clone());
